# most recent version of Steve's script
# only minimally modified by Anna (to change directories)
...
# STM 2016-06-22 CASA 4.6.0 use_initweights
# STM 2016-06-23 CASA 4.6.0 fix call to first immath stokes='I'
# STM 2016-06-24 CASA 4.6.0 add use_extraflags for extra flagging
# STM 2016-07-05 CASA 4.6.0 enable mtmfs & multscale
# STM 2016-07-11 CASA 4.6.0 minor fixes
# STM 2016-07-12 CASA 4.6.0 big bug fix, uvtaper was set
#================================================================================
logbuffer = []
mytext = 'VLASS QuickLook submosaic TClean MFS JointMosaic'
myscriptvers = '2016-07-12 STM (4.6.0)'
...
if 'use_multiscale' in locals() or 'use_multiscale' in globals():
    ...
    do_multiscale=False
#
if fld_nterms>1 or do_multiscale:
    # If multi-Taylor term and/or multiscale
...
# Re-weight before imaging?
do_initweights=False
my_initweights_wtmode=''
if 'use_initweights' in locals() or 'use_initweights' in globals():
    # run initweights before imaging
    if type(use_initweights) is types.BooleanType:
        do_initweights = use_initweights
        my_initweights_wtmode = 'nyq'
    elif type(use_initweights) is types.StringType:
        # extract the mode to use
        do_initweights=True
        my_initweights_wtmode = use_initweights
    elif type(use_initweights) is types.FloatType:
        do_initweights=True
        my_initweights_wtmode = 'ones'
    elif type(use_initweights) is types.IntType:
        do_initweights=True
        my_initweights_wtmode = 'ones'
...
if doimaging:
    ...
    #
    # Make a listing
    listobs(visname,listfile=visname+'.listobs',overwrite=True)
    #
    currTime=time.time()
    stagedur = currTime-prevTime
    # stepname = 'split'
    ...
    #
    # If requested re-initialize weights in ms
    if do_initweights:
        initweights(visname,wtmode=my_initweights_wtmode)
        #
        logstring = 'Re-Initialize MS weights using wtmode='+my_initweights_wtmode+' for '+visname
        print logstring
        casalog.post(logstring)
        logbuffer.append(logstring)
        #
        ...
        if steptimes.has_key(stepname):
            ...
            steplist.append(stepname)
            steptimes[stepname]=stagedur
        stagestr = 're-initialize weights'
        stagetime.append(stagedur)
        stagename.append(stagestr)
        print stagestr+' took '+str(stagedur)+' sec'
        prevTime = currTime
    ...
    #
    # If requested do extra flagging
    if 'use_extraflags' in locals() or 'use_extraflags' in globals():
        logstring = 'Applying extra flagging = '+str(use_extraflags)
        print logstring
        casalog.post(logstring)
        #
        flagdata(vis=visname,mode='list',inpfile=use_extraflags,action='apply',flagbackup=False)
        #
        currTime=time.time()
        stagedur = currTime-prevTime
        # stepname = 'split'
